...
TEST_CASE("AddressZeroPageX wraps within the zero page instead of crossing "
          "into page 1") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AddressZeroPageY wraps within the zero page instead of crossing "
          "into page 1") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AddressRelative subtracts a negative signed offset from the Program "
          "Counter") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AddressIndirect emulates the page-boundary bug when the pointer's "
          "low byte is 0xFF") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AddressIndirectX adds X to the zero page base, then reads a 16-bit "
          "pointer") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AddressIndirectX wraps the high-byte pointer fetch within the zero page") {
  ...
  bus.WriteCpu(0x00,
               0xF0); // base operand, also read back as the wrapped high byte
...
TEST_CASE("AddressIndirectY reads a zero page pointer, then adds Y to it") {
  ...
  bus.WriteCpu(0x00,
               0x20); // base operand, used directly as the zero page pointer
...
TEST_CASE("AddressIndirectY wraps the high-byte pointer fetch within the zero page") {
  ...
  bus.WriteCpu(0x00,
               0xFF); // base operand, also read back as the wrapped high byte
