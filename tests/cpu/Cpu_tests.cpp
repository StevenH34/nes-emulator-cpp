...
TEST_CASE("LdaImmediate updates the Zero and Negative flags based on the "
          "loaded value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Nmi pushes the Program Counter and a masked status register onto "
          "the stack") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Nmi loads the Program Counter from the NMI vector") {
  ...
  // NMI vector lives in the last 6 bytes of the 16 KB PRG bank (CPU
  // 0xFFFA/0xFFFB, mirrored down to PRG offset 0x3FFA/0x3FFB by Mapper000's 16
  // KB mask).
