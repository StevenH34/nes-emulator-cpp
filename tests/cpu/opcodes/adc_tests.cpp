...
TEST_CASE("AdcZeroPageX adds the value at the zero page address plus X to the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AdcAbsoluteX adds the value at a 16-bit address plus X to the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AdcAbsoluteY adds the value at a 16-bit address plus Y to the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AdcIndirectX adds X to the zero page base, reads a pointer, then "
          "adds the value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
