...
TEST_CASE("Tax copies the accumulator into the X register and updates the Zero "
          "and Negative flags") {
...
TEST_CASE("Tay copies the accumulator into the Y register and updates the Zero "
          "and Negative flags") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Txa copies the X register into the accumulator and updates the Zero "
          "and Negative flags") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Tya copies the Y register into the accumulator and updates the Zero "
          "and Negative flags") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Tsx copies the stack pointer into the X register and updates the "
...
TEST_CASE("Txs copies the X register into the stack pointer without affecting "
          "flags") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
  ...
  // Confirm the stack pointer actually moved by observing where the next push
  // lands.
