...
TEST_CASE("Ror returns the rotated value without touching memory or the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("RorZeroPage rotates the value at the zero page address and stores "
          "the result") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("RorAbsolute rotates the value at a 16-bit address and sets Zero and "
          "Carry") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
