...
TEST_CASE("EorZeroPage XORs the value at the zero page address with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("EorZeroPageX XORs the value at the zero page address plus X with "
          "the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("EorAbsoluteX XORs the value at a 16-bit address plus X with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("EorAbsoluteY XORs the value at a 16-bit address plus Y with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("EorIndirectX adds X to the zero page base, reads a pointer, then "
          "XORs the value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
