...
TEST_CASE("Compare sets Carry and clears Zero and Negative when register value "
...
TEST_CASE("Compare clears Carry and sets Negative when register value is less "
          "than operand") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpImmediate compares the accumulator with the value following the "
          "opcode") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpxImmediate compares the X register with the value following the "
          "opcode") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpxZeroPage compares the X register with the value at the zero page "
          "address") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpxZeroPage clears Carry when the X register is less than the zero "
          "page value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpxAbsolute clears Carry when the X register is less than the "
          "absolute value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpyImmediate compares the Y register with the value following the "
          "opcode") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpZeroPage compares the accumulator with the value at the zero "
          "page address") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpZeroPage clears Carry when the accumulator is less than the zero "
          "page value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpZeroPageX compares the accumulator with the value at the zero "
          "page address plus X") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpZeroPageX clears Carry when the accumulator is less than the "
          "indexed zero page value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
  ...
  bus.WriteCpu(0x22,
               0x10); // value at 0x20 + 0x02, 0x05 - 0x10 = 0xF5 (bit 7 set)
...
TEST_CASE("CmpAbsolute clears Carry when the accumulator is less than the "
          "absolute value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpAbsoluteX compares the accumulator with the value at a 16-bit "
          "address plus X") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpAbsoluteX clears Carry when the accumulator is less than the "
          "indexed absolute value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
  ...
  bus.WriteCpu(0x0508,
               0x10); // value at 0x0500 + 0x08, 0x05 - 0x10 = 0xF5 (bit 7 set)
...
TEST_CASE("CmpAbsoluteY compares the accumulator with the value at a 16-bit "
          "address plus Y") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpAbsoluteY clears Carry when the accumulator is less than the "
          "Y-indexed absolute value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
  ...
  cpu.SetYRegister(0x05);
  bus.WriteCpu(0x0705,
               0x10); // value at 0x0700 + 0x05, 0x05 - 0x10 = 0xF5 (bit 7 set)
...
TEST_CASE("CmpIndirectX adds X to the zero page base, reads a pointer, then "
          "compares with the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpIndirectX clears Carry when the accumulator is less than the "
          "indirectly addressed value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpIndirectY reads a zero page pointer, adds Y, then compares with "
          "the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CmpIndirectY clears Carry when the accumulator is less than the "
          "Y-offset indirect value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
  ...
  bus.WriteCpu(0x0608,
               0x10); // value at 0x0600 + 0x08, 0x05 - 0x10 = 0xF5 (bit 7 set)
...
TEST_CASE("CpyZeroPage compares the Y register with the value at the zero page "
          "address") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpyZeroPage clears Carry when the Y register is less than the zero "
          "page value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("CpyAbsolute clears Carry when the Y register is less than the "
          "absolute value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
