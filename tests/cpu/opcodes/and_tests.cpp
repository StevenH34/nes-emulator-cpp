...
TEST_CASE("AndZeroPage ANDs the value at the zero page address with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AndZeroPageX ANDs the value at the zero page address plus X with "
          "the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AndAbsoluteX ANDs the value at a 16-bit address plus X with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AndAbsoluteY ANDs the value at a 16-bit address plus Y with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("AndIndirectX adds X to the zero page base, reads a pointer, then "
          "ANDs the value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
