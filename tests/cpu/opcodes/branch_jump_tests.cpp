...
TEST_CASE("Beq does not branch but still consumes the offset byte when the "
          "Zero flag is clear") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("JmpIndirect sets the Program Counter to the address stored at the "
          "pointer") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("JmpIndirect reproduces the 6502 page-boundary bug") {
  ...
  bus.WriteCpu(0x0200,
               0x01); // target high byte read from the wrapped page start
...
TEST_CASE("Jsr pushes the return address and sets the Program Counter to the "
          "target") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Rts pops the return address from the stack and sets the Program "
          "Counter to address + 1") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Brk pushes the Program Counter and Status Register, sets the "
          "Interrupt flag, and jumps to the IRQ vector "
          "address") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Rti restores the Program Counter and Status Register from the "
          "stack, clearing the Break flag") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);

  cpu.StackPushWord(0x1234); // Program Counter, as Brk would have pushed it
  cpu.StackPushByte(0xFF); // Status Register, as Brk would have pushed it (all
                           // flags set, including Break)
...
TEST_CASE("Brk followed by Rti returns to the Program Counter and Status "
          "Register from before the interrupt") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
