...
TEST_CASE("Lsr returns the shifted value without touching memory or the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("LsrZeroPage shifts the value at the zero page address and stores "
          "the result") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("LsrAbsolute shifts the value at a 16-bit address and sets Zero and "
          "Carry") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
