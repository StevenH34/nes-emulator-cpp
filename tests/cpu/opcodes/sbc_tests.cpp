...
TEST_CASE("Sbc sets Overflow when positive minus negative produces a negative "
          "result") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Sbc sets Overflow when negative minus positive produces a positive "
          "result") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcImmediate subtracts the value following the opcode from the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcImmediate clears Carry and sets Zero when result wraps to 0x00 "
          "with borrow") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcZeroPage subtracts the value at the zero page address from the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcZeroPageX subtracts the value at the zero page address plus X "
...
TEST_CASE("SbcAbsolute subtracts the value at a 16-bit address from the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcAbsoluteX subtracts the value at a 16-bit address plus X from "
          "the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcAbsoluteY subtracts the value at a 16-bit address plus Y from "
          "the accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcIndirectX adds X to the zero page base, reads a pointer, then "
          "subtracts the value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("SbcIndirectY reads a zero page pointer, adds Y, then subtracts the "
          "value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
