...
TEST_CASE("OraZeroPageX ORs the value at the zero page address plus X with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("OraAbsoluteX ORs the value at a 16-bit address plus X with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("OraAbsoluteY ORs the value at a 16-bit address plus Y with the "
          "accumulator") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("OraIndirectX adds X to the zero page base, reads a pointer, then "
          "ORs the value") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
