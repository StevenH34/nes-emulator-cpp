...
TEST_CASE("StackPushByte writes the value to the stack page and decrements the "
          "stack pointer") {
...
TEST_CASE("StackPushByte and StackPullByte round trip multiple values in LIFO "
          "order") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("StackPushWord writes the high byte then the low byte, decrementing "
...
TEST_CASE("StackPushWord and StackPullWord round trip multiple values in LIFO "
          "order") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Stack pointer wraps from 0x00 to 0xFF when pushing past the bottom "
          "of the stack") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Stack pointer wraps from 0xFF to 0x00 when popping past the top of "
          "the stack") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Pha pushes the accumulator onto the stack and decrements the stack "
          "pointer") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
...
TEST_CASE("Pla pops a value from the stack into the accumulator and updates "
          "the Negative flag") {
  nes_test::TestBus bus;
  nes::Cpu cpu(bus);
