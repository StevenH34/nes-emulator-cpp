...
TEST_CASE("WriteScroll's first write sets the X scroll and the second sets the "
          "Y scroll") {
  auto cart = MakeCartridge();
  nes::Ppu ppu(cart);
...
TEST_CASE("WriteAddr masks the first write's high byte to 6 bits (14-bit "
          "address space)") {
  auto cart = MakeCartridge();
  nes::Ppu ppu(cart);
...
TEST_CASE("ReadDataRegister on a palette address refills the buffer from "
          "0x1000 below") {
  auto cart = MakeCartridge();
  nes::Ppu ppu(cart);
...
TEST_CASE("WriteVram/ReadVram share physical nametable RAM across mirrored "
          "addresses") {
...
TEST_CASE("MirrorNametableAddr treats nametables 0 and 1 identically under "
...
TEST_CASE("MirrorNametableAddr treats nametables 0 and 2 identically under "
          "vertical mirroring") {
...
TEST_CASE("PaletteIndex mirrors sprite palette 'color 0' entries into the "
          "background palette") {
...
TEST_CASE("WriteVram/ReadVram share palette RAM across the sprite/background "
          "mirror") {
  auto cart = MakeCartridge();
  nes::Ppu ppu(cart);
