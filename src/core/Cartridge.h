...
public:
  ...
  static constexpr std::array<uint8_t, 4> NES_HEADER = {0x4E, 0x45, 0x53, 0x1A}; // "NES\x1A"
  // ROM block sizes
  static constexpr uint32_t PRG_BLOCK_SIZE = 0x4000; // 16 KB
  static constexpr uint32_t CHR_BLOCK_SIZE = 0x2000; // 8 KB
  // Trainer
  static constexpr uint32_t TRAINER_SIZE = 512;
  // Header byte positions
  static constexpr uint32_t PRG_SIZE_BYTE = 4;
  static constexpr uint32_t CHR_SIZE_BYTE = 5;
  static constexpr uint32_t FLAGS_6_BYTE = 6;
  static constexpr uint32_t FLAGS_7_BYTE = 7;
  // Flags 6 masks
  static constexpr uint8_t MIRROR_MASK = 0b0000'0001; // 0 = horizontal, 1 = vertical
  static constexpr uint8_t BATTERY_MASK = 0b0000'0010; // 1 = battery-backed RAM
  static constexpr uint8_t TRAINER_MASK = 0b0000'0100; // 1 = 512-byte trainer before PRG
  static constexpr uint8_t FOUR_SCREEN_MASK = 0b0000'1000; // 1 = four-screen VRAM
  ...
  [[nodiscard]] const std::vector<uint8_t>& GetChrRom() const { return chr_rom_; }
