#pragma once

#include "ApuConstants.h"
#include "Noise.h"
#include "Pulse.h"
#include "Triangle.h"

#include <cstdint>
#include <vector>

namespace nes {

class Apu {
public:
  Apu();
  ~Apu() = default;

  [[nodiscard]] Pulse& GetPulse1() { return pulse1_; }
  [[nodiscard]] Pulse& GetPulse2() { return pulse2_; }
  [[nodiscard]] Triangle& GetTriangle() { return triangle_; }
  [[nodiscard]] Noise& GetNoise() { return noise_; }

  [[nodiscard]] uint8_t GetFrameMode() const { return frame_mode_; }
  [[nodiscard]] int32_t GetFrameCycle() const { return frame_cycle_; }
  [[nodiscard]] float GetSampleClock() const { return sample_clock_; }
  [[nodiscard]] std::vector<float>& GetSampleBuffer() { return sample_buffer_; }
  [[nodiscard]] uint32_t GetCycle() const { return cycle_; }

  void Step();

private:
  // Channels
  Pulse pulse1_;
  Pulse pulse2_;
  Triangle triangle_;
  Noise noise_;

  // 0 = 4-step sequence (~60Hz), 1 = 5-step sequence (~48Hz)
  uint8_t frame_mode_{0};
  // Current cycle within the frame counter sequence
  int32_t frame_cycle_{0};
  // CPU cycle counter; even/odd determines APU timing
  uint32_t cycle_{0};
  // Accumulator for downsampling from ~1.79MHz to 44100Hz
  float sample_clock_{0.0f};
  // Float32 samples; GUI drains them each frame (~735 per frame)
  std::vector<float> sample_buffer_;

  static constexpr uint32_t SAMPLE_RATE = 44100;
  static constexpr uint32_t CPU_CLOCK = 1789773;
  // Used for downsampling
  static constexpr float CYCLES_PER_SAMPLE = static_cast<float>(CPU_CLOCK) / SAMPLE_RATE;
  // Pulse 1 registers
  static constexpr uint16_t PULSE1_MAIN_REGISTER = 0x4000;
  static constexpr uint16_t PULSE1_SWEEP_REGISTER = 0x4001;
  static constexpr uint16_t PULSE1_TIMER_LOW_REGISTER = 0x4002;
  static constexpr uint16_t PULSE1_TIMER_HIGH_REGISTER = 0x4003;
  // Pulse 2 registers
  static constexpr uint16_t PULSE2_MAIN_REGISTER = 0x4004;
  static constexpr uint16_t PULSE2_SWEEP_REGISTER = 0x4005;
  static constexpr uint16_t PULSE2_TIMER_LOW_REGISTER = 0x4006;
  static constexpr uint16_t PULSE2_TIMER_HIGH_REGISTER = 0x4007;
  // Triangle registers
  static constexpr uint16_t TRIANGLE_LINEAR_REGISTER = 0x4008;
  static constexpr uint16_t TRIANGLE_TIMER_LOW_REGISTER = 0x400A;
  static constexpr uint16_t TRIANGLE_TIMER_HIGH_REGISTER = 0x400B;
  // Noise registers
  static constexpr uint16_t NOISE_MAIN_REGISTER = 0x400C;
  static constexpr uint16_t NOISE_MODE_PERIOD_REGISTER = 0x400E;
  static constexpr uint16_t NOISE_LENGTH_REGISTER = 0x400F;
  // Frame counter step sequence
  struct FrameCounterNtsc {
    // Mode 0: 4-Step Sequence
    static constexpr uint32_t MODE0_STEP1 = 7457; // 3728.5 * 2
    static constexpr uint32_t MODE0_STEP2 = 14914; // 7456.5 * 2 (Rounded)
    static constexpr uint32_t MODE0_STEP3 = 22371; // 11185.5 * 2
    static constexpr uint32_t MODE0_STEP4 = 29829; // 14914.5 * 2
    static constexpr uint32_t MODE0_MAX = 29830; // Total sequence length

    // Mode 1: 5-Step Sequence
    static constexpr uint32_t MODE1_STEP1 = 7457; // 3728.5 * 2
    static constexpr uint32_t MODE1_STEP2 = 14914; // 7456.5 * 2
    static constexpr uint32_t MODE1_STEP3 = 22371; // 11185.5 * 2
    static constexpr uint32_t MODE1_STEP4 = 29829; // 14914.5 * 2
    static constexpr uint32_t MODE1_STEP5 = 37281; // 18640.5 * 2
    static constexpr uint32_t MODE1_MAX = 37282; // Total sequence length
  };
  // Status register bit masks
  static constexpr uint8_t STATUS_PULSE1_MASK = 0x01;
  static constexpr uint8_t STATUS_PULSE2_MASK = 0x02;
  static constexpr uint8_t STATUS_TRIANGLE_MASK = 0x04;
  static constexpr uint8_t STATUS_NOISE_MASK = 0x08;
  // Mixer Coefficients
  static constexpr float PULSE_MIXER_COEFFICIENT = 0.00752f;
  static constexpr float TND_T_MIXER_COEFFICIENT = 0.00851f;
  static constexpr float TND_N_MIXER_COEFFICIENT = 0.00494f;
};

} // namespace nes